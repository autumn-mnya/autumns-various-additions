...
signed char File_ReadS8(FILE* stream)
{
	signed char byte;
...
void File_WriteLE8(signed char value, FILE* stream)
{
...
unsigned long Freeware_ReadBE32(FILE* stream)
{
	unsigned char bytes[4];

	Freeware_fread(bytes, 4, 1, stream);

	return (bytes[0] << 24) | (bytes[1] << 16) | (bytes[2] << 8) | bytes[3];
}

unsigned long Freeware_ReadBE64(FILE* stream)
{
	unsigned char bytes[8];

	Freeware_fread(bytes, 8, 1, stream);

	return (bytes[0] << 56) | (bytes[1] << 48) | (bytes[2] << 40) | (bytes[3] << 32) | (bytes[4] << 24) | (bytes[5] << 16) | (bytes[6] << 8) | bytes[7];
}


unsigned long Freeware_ReadLE32(FILE* stream)
{
	unsigned char bytes[4];
...
unsigned long Freeware_ReadLE64(FILE* stream)
{
	unsigned char bytes[8];

	Freeware_fread(bytes, 8, 1, stream);

	return (bytes[7] << 56) | (bytes[6] << 48) | (bytes[5] << 40) | (bytes[4] << 32) | (bytes[3] << 24) | (bytes[2] << 16) | (bytes[1] << 8) | bytes[0];
}

unsigned long Freeware_WriteLE32(FILE* stream)
{
	unsigned char bytes[4];

	Freeware_fwrite(bytes, 4, 1, stream);

	return (bytes[3] << 24) | (bytes[2] << 16) | (bytes[1] << 8) | bytes[0];
}

void Freeware_WriteLE32(unsigned int value, FILE* stream)
{
	int b;
	for (unsigned int i = 0; i < 4; ++i)
	{
		b = value >> (8 * i);
		Freeware_fwrite(&b, 1, 1, stream); // jank as hell.
	}
}