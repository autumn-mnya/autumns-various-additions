...
void InitMod_Settings()
{
	...
	char path2[MAX_PATH];
	sprintf(path2, "%s\\%s", gModulePath, "config_test.ini");
	INIReader test(path2);

	bool testB = test.GetBoolean("Test", "bool", false);
	int testI = test.GetInteger("Test", "int", 0);

	printf("%s%d%s", "Resulting outcome for boolean is ", testB, "\n");
	printf("%s%d%s", "Resulting outcome for integer is ", testI, "\n");
}
